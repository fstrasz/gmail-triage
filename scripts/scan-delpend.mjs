...
const creds = JSON.parse(
  fs.readFileSync(path.join(CONFIG_DIR, "credentials.json")),
);
const token = JSON.parse(fs.readFileSync(path.join(CONFIG_DIR, "token.json")));
const blocklist = JSON.parse(
  fs.readFileSync(path.join(CONFIG_DIR, "blocklist.json")),
);
...
if (!delPendLabel) {
  console.error(".DelPend label not found");
  process.exit(1);
}
...
do {
  ...
  for (let i = 0; i < messages.length; i += 50) {
    ...
    for (const hdrs of headers) {
      ...
      if (!email) continue;
      if (!fromMap.has(email))
        fromMap.set(email, { email, name, count: 0, subjects: [] });
