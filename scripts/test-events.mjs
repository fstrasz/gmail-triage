...
test("gmail message url regex: rejects malformed and non-Gmail URLs", () => {
  ...
  assert.equal(
    null,
    "https://mail.google.com/mail/u/0/#all/xyz123".match(RE),
    "non-hex ID",
  );
...
test('subject filter: "Gmail Triage" includes() catches both self-sent variants', () => {
  const isSelfReferential = (subject) =>
    (subject || "").includes("Gmail Triage");
  assert.ok(isSelfReferential("Gmail Triage - Upcoming Events (May 15, 2026)"));
  assert.ok(
    isSelfReferential("Gmail Triage - Daily Auto-Clean Summary (May 15, 2026)"),
  );
...
test("view-in-browser regex: matches common phrasings", () => {
  const RE =
    /\b(view\s+(this\s+)?(email|message)?\s*(in\s+(your\s+|a\s+)?browser|as\s+(a\s+)?web\s*page|online|in\s+a\s+browser)|trouble\s+viewing|having\s+trouble\s+viewing|see\s+(it|this)\s+in\s+your\s+browser)\b/i;
...
test("source-icon predicate: shown only when source=email AND no canonicalUrl", () => {
  ...
  assert.ok(
    shouldShowEmailIcon({ source: "email", url: "gmail-url" }),
    "email-source without canonical → icon",
  );
  assert.ok(
    !shouldShowEmailIcon({
      source: "email",
    ...
    "email-source WITH canonical → NO icon",
  );
  assert.ok(
    shouldShowEmailIcon({ source: "email", canonicalUrl: "" }),
    "empty-string canonical is falsy → icon shows",
  );
  assert.ok(
    !shouldShowEmailIcon({ source: "web" }),
    "non-email source → no icon",
  );
...
test("noise filter: rejects unsubscribe + social hosts", () => {
  ...
  const isNoise = (a) => {
    ...
    try {
      return NOISE_HOST.some((p) => p.test(new URL(a.href).host));
    } catch {
      return true;
    }
  };
  ...
  assert.ok(
    !isNoise({
      href: "https://localwineevents.com/c/abc",
      text: "Get Tickets",
    }),
  );
});
...
test("batchByTokenBudget: packs greedily, opens new batch when budget would be exceeded", async () => {
  ...
  assert.deepEqual(
    batches[0].map((b) => b.id),
    ["a", "b"],
  );
  assert.deepEqual(
    batches[1].map((b) => b.id),
    ["c", "d"],
  );
});
...
test("batchByTokenBudget: empty input → empty batches", async () => {
  const { batchByTokenBudget } = await import(claudeUtilsModulePath);
  assert.deepEqual(
    batchByTokenBudget([], (i) => i.tokens, 10_000),
    [],
  );
});
...
test("batchByTokenBudget: max-count Infinity (default) keeps old behavior", async () => {
  ...
  assert.equal(
    batches.length,
    1,
    "50 items × 10 tokens = 500 tokens, all fit, default no-count-cap",
  );
});
...
test("scoreCandidateByTitle: URL contains all meaningful words → high score", async () => {
  ...
  const s = scoreCandidateByTitle(
    {
      href: "https://x.ticketspice.com/hands-on-pasta-making-2026",
      text: "TICKETS",
    },
...
test("scoreCandidateByTitle: URL with no overlap → 0", async () => {
  const { scoreCandidateByTitle } = await import(eventSearchModulePath);
  assert.equal(
    scoreCandidateByTitle(
      { href: "https://x.com/random/path", text: "" },
      "Hands-On Pasta Making Class",
    ),
    0,
  );
...
test("scoreCandidateByTitle: empty/stopword title → 0", async () => {
  const { scoreCandidateByTitle } = await import(eventSearchModulePath);
  assert.equal(
    scoreCandidateByTitle({ href: "https://x.com", text: "TICKETS" }, ""),
    0,
  );
  assert.equal(
    scoreCandidateByTitle(
      { href: "https://x.com", text: "TICKETS" },
      "The And Of",
    ),
    0,
  );
...
test("pickCanonicalFromCandidates: single candidate → kind direct (no Claude)", async () => {
  const { pickCanonicalFromCandidates } = await import(eventSearchModulePath);
  const r = pickCanonicalFromCandidates(
    [{ href: "https://x.com/only", text: "TICKETS" }],
    "Event",
  );
...
test("pickCanonicalFromCandidates: ambiguous returns ranked + capped candidates", async () => {
  ...
  const cands = Array.from({ length: 100 }, (_, i) => ({
    href: "https://x.com/p" + i,
    text: "Link " + i,
  }));
  const r = pickCanonicalFromCandidates(cands, "Untitled", {
    maxClaudeCandidates: 10,
  });
  assert.equal(r.kind, "ambiguous");
...
test("scoreCandidateByTitle: ranking puts target URL first among many", async () => {
  ...
  const ranked = candidates
    .map((c) => ({
      ...c,
      score: scoreCandidateByTitle(c, "Hands-On Pasta Making Class"),
    }))
    .sort((a, b) => b.score - a.score);
  assert.equal(
    ranked[0].href,
    "https://x.com/hands-on-pasta-making-2026",
    "pasta URL bubbles to top",
  );
});
...
test("sortGroupKeysByLocationOrder: empty configured → pure alphabetical", async () => {
  const { sortGroupKeysByLocationOrder } = await import(eventSearchModulePath);
  const out = sortGroupKeysByLocationOrder(
    ["Temecula, CA", "Las Vegas, NV", "Other"],
    [],
  );
...
test("getEmailBodyText: stripped-html longer than plain → returns stripped html (The Juice case)", async () => {
  ...
  const plain =
    "Hi Frank. Here is a list of upcoming events.\nLink: https://example.com";
  const html =
    "<html><body>" +
    Array.from(
      { length: 16 },
      (_, i) =>
        `<div>May ${15 + (i % 3)}, 2026 (Fri)<br>Event ${i + 1}: Wine Tasting</div>`,
    ).join("") +
    "</body></html>";
  const out = getEmailBodyText(makePayload({ plain, html }));
  assert.ok(
    out.includes("Event 1: Wine Tasting"),
    "should contain html-derived event listings",
  );
...
test("getEmailBodyText: only html → returns stripped html", async () => {
  const { getEmailBodyText } = await import(eventSearchModulePath);
  const out = getEmailBodyText(
    makePayload({ html: "<p>Hello <strong>world</strong></p>" }),
  );
...
test("extractJsonArray: array with preamble and postamble", async () => {
  const { extractJsonArray } = await import(claudeUtilsModulePath);
  assert.equal(
    extractJsonArray("Here you go: [1,2,3] hope that helps"),
    "[1,2,3]",
  );
...
test("extractBodyScanEvents: empty events array from tool_use is preserved (not a failure)", async () => {
  ...
  const out = extractBodyScanEvents({
    content: [
      { type: "tool_use", name: "record_events", input: { events: [] } },
    ],
  });
  assert.equal(out.source, "tool_use");
  ...
  try {
    const initial = [
      ...
      },
      {
        id: "2",
        title: "Trashed",
        url: "https://mail.google.com/mail/u/0/#all/bbb222",
        ignored: false,
      },
      {
        id: "3",
        title: "Purged",
        url: "https://mail.google.com/mail/u/0/#all/ccc333",
        ignored: false,
      },
      {
        id: "4",
        title: "Self-ref",
        url: "https://mail.google.com/mail/u/0/#all/ddd444",
        ignored: false,
      },
      {
        id: "5",
      ...
      {
        id: "6",
        title: "AlreadyIgnored",
        url: "https://mail.google.com/mail/u/0/#all/eee555",
        ignored: true,
      },
    ];
      ...
      users: {
        messages: {
          get: async ({ id }) => {
            if (id === "aaa111")
              return {
                data: {
                  ...
                  payload: {
                    headers: [
                      { name: "Subject", value: "Mother's Day brunch" },
                    ],
                  },
                },
              };
            if (id === "bbb222")
              return {
                data: {
                  labelIds: ["TRASH"],
                  payload: {
                    headers: [{ name: "Subject", value: "Old event" }],
                  },
                },
              };
            if (id === "ccc333") {
                ...
                data: {
                  ...
                  payload: {
                    headers: [
                      {
                        name: "Subject",
                        value: "Gmail Triage - Upcoming Events (May 14, 2026)",
                      },
                    ],
                  },
                },
            ...
            if (id === "eee555") {
              throw new Error(
                "should never be called: already-ignored event must be skipped",
              );
            }
...
test("dedupKey: Gmail message URL keyed by (url, title, date)", async () => {
  ...
  assert.notEqual(
    k1,
    k2,
    "different titles → different keys for same Gmail URL",
  );
...
test("dedupKey: missing url falls back to title|date", async () => {
  const { dedupKey } = await import(foundEventsModulePath);
  assert.equal(
    dedupKey({ title: "Holiday Party", date: "2026-12-15" }),
    "Holiday Party|2026-12-15",
  );
});

test("upsertFoundEvents: 16 events from same Gmail URL all preserved (newsletter case)", async () => {
  const tmpDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "gmail-triage-upsert-test-"),
  );
  const origCwd = process.cwd();
  process.chdir(tmpDir);
  try {
...
test("collectMatchingIds: walks all pages and harvests every id", async () => {
  ...
  assert.deepEqual(await collectMatchingIds(gmail, "from:x in:inbox"), [
    "a",
    "b",
    "c",
    "d",
...
test("collectMatchingIds: empty page returns []", async () => {
  const { collectMatchingIds } = await import(gmailModulePath);
  const gmail = {
    users: { messages: { list: async () => ({ data: { messages: [] } }) } },
  };
        ...
        list: async () => ({
          data: {
            labels: [
              { name: "..VIP", id: "LBL_VIP" },
              { name: "..OK", id: "LBL_OK" },
              { name: ".DelPend", id: "LBL_DEL" },
              { name: "MyRule", id: "LBL_RULE" },
            ],
          },
        }),
      },
...
test("undoReapply (tier): strips the tier label, re-adds nothing, markedUnread false", async () => {
  ...
  const g = reapplyMockGmail();
  const { reversed, markedUnread } = await undoReapply(g, {
    batches: [
      { addLabelIds: ["..VIP"], removeLabelIds: [], ids: ["m1", "m2"] },
    ],
  });
...
test("undoReapply (blocklist): re-adds INBOX+UNREAD, strips .DelPend, flags markedUnread", async () => {
  const { undoReapply } = await import(gmailModulePath);
  const g = reapplyMockGmail();
  const { markedUnread } = await undoReapply(g, {
    batches: [
      {
        addLabelIds: [".DelPend"],
...
test("undoReapply chunks ids at <=1000 per batchModify call", async () => {
  ...
  await undoReapply(g, {
    batches: [{ addLabelIds: ["..VIP"], removeLabelIds: [], ids }],
  });
...
function keepCleanMockGmail(listedIds) {
  ...
  return {
    ...
    users: {
      labels: {
        list: async () => ({
          data: { labels: [{ name: ".DelPend", id: "LBL_DEL" }] },
        }),
      },
      messages: {
        list: async (params) => {
          queries.push(params.q);
          return { data: { messages: listedIds.map((id) => ({ id })) } };
        },
        batchModify: async (p) => {
...
test("keepAndClean: sweep query excludes ..VIP and does not exclude ..OK", async () => {
  ...
  assert.ok(
    !q.includes("..OK"),
    "a ..OK exclusion was re-added; see the retired Phase 9 criterion in roadmap.md before changing this",
  );
});
...
test("setLastReapply / clearLastReapply round-trip through settings.json", async () => {
  ...
  process.chdir(dir);
  try {
    const { setLastReapply, clearLastReapply, loadSettings } = await import(
    ...
    const rec = {
      list: "vip",
      ts: "x",
      batches: [{ addLabelIds: ["..VIP"], removeLabelIds: [], ids: ["m1"] }],
    };
...
test("getBulkGuardThreshold: returns the fallback when unset", async () => {
  ...
  process.chdir(dir);
  try {
    const { getBulkGuardThreshold } = await import(
      settingsModulePath + "?t=" + Date.now() + Math.random()
    );
...
test("getBulkGuardThreshold: a positive setting overrides the fallback", async () => {
  ...
  try {
    fs.writeFileSync(
      "settings.json",
      JSON.stringify({ bulkGuardThreshold: 250 }),
    );
    const { getBulkGuardThreshold } = await import(
      settingsModulePath + "?t=" + Date.now() + Math.random()
    );
...
test("getBulkGuardThreshold: invalid setting (0 / non-number) falls back to the constant", async () => {
  ...
  try {
    fs.writeFileSync(
      "settings.json",
      JSON.stringify({ bulkGuardThreshold: 0 }),
    );
    ...
    fs.writeFileSync(
      "settings.json",
      JSON.stringify({ bulkGuardThreshold: "lots" }),
    );
...
test("setBulkGuardThreshold: a positive number stores the floored value", async () => {
  ...
  process.chdir(dir);
  try {
    const { setBulkGuardThreshold, getBulkGuardThreshold, loadSettings } =
      await import(settingsModulePath + "?t=" + Date.now() + Math.random());
    setBulkGuardThreshold(250.9);
    assert.equal(
      loadSettings().bulkGuardThreshold,
      250,
      "stores Math.floor of the value",
    );
    assert.equal(
      getBulkGuardThreshold(100),
      250,
      "getter returns the set value over the fallback",
    );
  } finally {
    process.chdir(origCwd);
    fs.rmSync(dir, { recursive: true, force: true });
  ...
  try {
    ...
    for (const bad of [0, -5, NaN, Infinity, "lots", null, undefined]) {
      ...
      assert.equal(
        loadSettings().bulkGuardThreshold,
        undefined,
        `${String(bad)} clears the key`,
      );
      assert.equal(
        getBulkGuardThreshold(100),
        100,
        `${String(bad)} → getter falls back to default`,
      );
    }
  } finally {
    process.chdir(origCwd);
...
test('extractEmail: "Name <addr>" form returns lowercased addr', async () => {
  ...
  assert.equal(
    extractEmail("Frank Strasz <frank@example.com>"),
    "frank@example.com",
  );
  assert.equal(
    extractEmail('"Last, First" <ll@example.com>'),
    "ll@example.com",
  );
...
test("extractEmail: plus-tag preserved", async () => {
  const { extractEmail } = await import(gmailModulePath);
  assert.equal(
    extractEmail("frank+newsletter@example.com"),
    "frank+newsletter@example.com",
  );
  assert.equal(
    extractEmail("Newsletter <frank+ml@example.com>"),
    "frank+ml@example.com",
  );
});
...
test("sanitizeUrl: accepts public http/https URLs", async () => {
  ...
  assert.equal(
    sanitizeUrl("https://example.com/unsub"),
    "https://example.com/unsub",
  );
  assert.equal(
    sanitizeUrl("http://example.com/unsub"),
    "http://example.com/unsub",
  );
  assert.equal(
    sanitizeUrl("https://list-manage.com/unsubscribe?u=123"),
    "https://list-manage.com/unsubscribe?u=123",
  );
});
...
test("list matchers: isBlocked / isViplisted / isOklisted contracts", async () => {
  const sharedDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "gmail-triage-list-test-"),
  );
  const origCwd = process.cwd();
  ...
  try {
    ...
    const write = (name, data) =>
      fs.writeFileSync(path.join(sharedDir, name), JSON.stringify(data));
    const clear = (name) => {
    ...
    assert.ok(
      !isBlocked("other@example.com"),
      "isBlocked: non-match returns falsy",
    );

    // case-insensitive and trim
    assert.ok(isBlocked("SPAM@EXAMPLE.COM"), "isBlocked: uppercase normalized");
    assert.ok(
      isBlocked("  spam@example.com  "),
      "isBlocked: whitespace trimmed",
    );
    ...
    assert.ok(
      !isBlocked("anyone@goodcorp.com"),
      "isBlocked: different domain not matched",
    );

    // name-scoped entry matches same name; unscoped caller still matches
    write("blocklist.json", [
      { email: "shared@example.com", name: "Spammer One" },
    ]);
    assert.ok(
      isBlocked("shared@example.com", "Spammer One"),
      "isBlocked: name match",
    );
    assert.ok(
      !isBlocked("shared@example.com", "Different Sender"),
      "isBlocked: name mismatch",
    );
    assert.ok(
      isBlocked("shared@example.com"),
      "isBlocked: no name from caller still matches",
    );

    // isViplisted returns boolean
    write("viplist.json", [{ email: "vip@example.com" }]);
    assert.equal(
      isViplisted("vip@example.com"),
      true,
      "isViplisted: returns true",
    );
    assert.equal(
      isViplisted("other@example.com"),
      false,
      "isViplisted: returns false",
    );

    // isOklisted returns boolean
    write("oklist.json", [{ email: "ok@example.com" }]);
    assert.equal(
      isOklisted("ok@example.com"),
      true,
      "isOklisted: returns true",
    );
    assert.equal(
      isOklisted("other@example.com"),
      false,
      "isOklisted: returns false",
    );
    ...
    assert.equal(
      isViplisted("any@example.com"),
      false,
      "isViplisted: missing file → false",
    );
    assert.equal(
      isOklisted("any@example.com"),
      false,
      "isOklisted: missing file → false",
    );
  } finally {
    process.chdir(origCwd);
...
test("senderList: factory load/save/add/remove/match contracts", async () => {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), "gmail-triage-factory-test-"),
  );
  const origCwd = process.cwd();
  process.chdir(dir);
  try {
    ...
    assert.equal(
      list.length,
      2,
      "name-scoped: a new display name is a new entry",
    );

    // add normalizes email (lowercase + trim)
    store.add("  B@X.com ");
    assert.ok(
      store.load().some((e) => e.email === "b@x.com"),
      "email normalized",
    );
...
test("senderList: name-scoped add keeps same-email different-name variants", async () => {
  ...
  try {
    ...
    assert.equal(
      store.load().length,
      2,
      "new name-variant is added, not a silent no-op",
    );
    assert.ok(
      store.match("nytdirect@nytimes.com", "NYT Cooking"),
      "new variant matches",
    );
    assert.ok(
      store.match("nytdirect@nytimes.com", "Bake Time"),
      "original variant still matches",
    );
...
test("shapeTriageEmail: derives fromEmail/fromName from raw From header", async () => {
  const { shapeTriageEmail } = await import(
    triageApiModulePath + "?t=" + Date.now() + Math.random()
  );
  ...
  assert.equal(
    shaped.hasUnsub,
    true,
    "hasUnsub true when listUnsubscribe present",
  );
  assert.equal(
    shaped.unsubUrl,
    "<https://unsub.nytimes.com/u>",
    "unsubUrl preserved",
  );
  assert.equal(
    shaped.unsubPost,
...
test("filterHidden: hideListed:true drops VIP, OK, and blocklisted senders; hideListed:false drops only blocked", async () => {
  ...
  try {
    fs.writeFileSync(
      path.join(dir, "viplist.json"),
      JSON.stringify([{ email: "vip@example.com" }]),
    );
    fs.writeFileSync(
      path.join(dir, "oklist.json"),
      JSON.stringify([{ email: "ok@example.com" }]),
    );
    fs.writeFileSync(
      path.join(dir, "blocklist.json"),
      JSON.stringify([{ email: "blocked@example.com", reason: "junk" }]),
    );

    const { filterHidden } = await import(
      triageApiModulePath + "?t=" + Date.now() + Math.random()
    );
...
test("isListedSender: true for VIP/OK/@domain listed senders, false otherwise", async () => {
  ...
  try {
    ...
    assert.equal(
      isListedSender("vip@example.com"),
      true,
      "VIP exact match → listed",
    );
    assert.equal(
      isListedSender("VIP@EXAMPLE.COM"),
      true,
      "VIP match is case-insensitive → listed",
    );
    assert.equal(
      isListedSender("ok@example.com"),
      true,
      "OK exact match → listed",
    );
    assert.equal(
      isListedSender("anyone@vipcorp.com"),
      true,
      "VIP @domain match → listed",
    );
    assert.equal(
      isListedSender("stranger@example.com"),
      false,
      "unlisted → not listed",
    );
    assert.equal(
...
test("activityLog: caps at 250 entries, newest first, compact JSON", async () => {
  ...
  process.chdir(dir);
  try {
    const { appendLog, loadLog } = await import(
      activityLogModulePath + "?t=" + Date.now() + Math.random()
    );
    for (let i = 0; i < 260; i++)
      appendLog({ type: "triage", action: "ok", seq: i });
    const log = loadLog();
...
test("atomicWriteFileSync: normal path writes content (temp + rename)", async () => {
  ...
  try {
    const { atomicWriteFileSync } = await import(
      atomicWriteModulePath + "?t=" + Date.now() + Math.random()
    );
...
test("atomicWriteFileSync: falls back to direct write when rename throws EBUSY (bind-mount case)", async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      "new",
      "fallback wrote content despite EBUSY",
    );
    assert.equal(
      fs.readdirSync(dir).filter((n) => n.includes(".tmp-")).length,
      0,
      "temp cleaned up after EBUSY",
    );
  } finally {
...
test("senderList: dedupeOnLoad true drops exact email+name dups, keeps name-variants; false keeps all", async () => {
  const dir = fs.mkdtempSync(
    path.join(os.tmpdir(), "gmail-triage-factory-dedup-"),
  );
  const origCwd = process.cwd();
  process.chdir(dir);
  try {
    ...
    const variants = [
      { email: "a@x.com", name: "One" },
      { email: "A@X.com", name: "Two" },
    ];
    ...
    assert.equal(
      kept.load().length,
      2,
      "dedupeOnLoad true → keeps same-email different-name variants",
    );
    ...
    assert.equal(
      deduped.load().length,
      1,
      "dedupeOnLoad true → exact email+name dup dropped",
    );
    ...
    assert.equal(
      raw.load().length,
      2,
      "dedupeOnLoad false → keeps even exact dups",
    );
  } finally {
    process.chdir(origCwd);
    fs.rmSync(dir, { recursive: true, force: true });
...
test("getHealthReport: healthy when token ok, config ok, scan fresh", async () => {
  ...
  const r = getHealthReport({
    ...
    settings: {
      ...
      schedulerLastRunAt: "2026-06-09T11:30:00Z",
    },
    tokenState: "ok",
    configState: "ok",
  });
...
test("getHealthReport: degraded when scans are stale (two missed intervals)", async () => {
  ...
  const r = getHealthReport({
    ...
    settings: {
      schedulerEnabled: true,
      schedulerIntervalHours: 2,
      schedulerLastRunAt: "2026-06-09T06:00:00Z",
    },
    tokenState: "ok",
    configState: "ok",
  });
...
test("setSchedulerLastRunAt: writes an ISO timestamp readable by loadSettings", async () => {
  ...
  process.chdir(dir);
  try {
    const { setSchedulerLastRunAt, loadSettings } = await import(
      settingsModulePath + "?t=" + Date.now() + Math.random()
    );
    ...
    assert.ok(
      stamped && !Number.isNaN(Date.parse(stamped)),
      "stamped a valid ISO timestamp",
    );
  } finally {
    process.chdir(origCwd);
    fs.rmSync(dir, { recursive: true, force: true });
...
test("senderList.remove: name-scoped removes only the exact pair; name-blind removes all", async () => {
  ...
  try {
    ...
    assert.equal(
      after.length,
      1,
      "name-scoped remove drops only the matching pair",
    );
...
test("untrashMessage: untrashes then re-adds INBOX via batchModify", async () => {
  ...
  const gmail = {
    users: {
      messages: {
        untrash: async (a) => {
          calls.push(["untrash", a]);
        },
        batchModify: async (a) => {
          calls.push(["batchModify", a]);
        },
      },
    },
  };
  ...
  assert.ok(
    calls[1][1].requestBody.addLabelIds.includes("INBOX"),
    "batchModify re-adds INBOX",
  );
});

// ─── getHealthReport: web-asset check (Task 9) ───────────────────────────────
test("getHealthReport: webAsset missing → degraded, checks.web = missing", async () => {
  const { getHealthReport } = await import(
    healthModulePath + "?webAsset1=" + Date.now()
  );
  ...
  const r = getHealthReport({
    ...
    settings: {
      schedulerEnabled: true,
      schedulerIntervalHours: 2,
      schedulerLastRunAt: "2026-06-23T11:30:00Z",
    },
    tokenState: "ok",
    configState: "ok",
    webAsset: "missing",
...
test("getHealthReport: webAsset ok → not degraded by web check", async () => {
  const { getHealthReport } = await import(
    healthModulePath + "?webAsset2=" + Date.now()
  );
  const now = Date.parse("2026-06-23T12:00:00Z");
  const r = getHealthReport({
    version: "v1.2.05",
...
test("getHealthReport: webAsset disabled → not degraded by web check", async () => {
  const { getHealthReport } = await import(
    healthModulePath + "?webAsset3=" + Date.now()
  );
  const now = Date.parse("2026-06-23T12:00:00Z");
  const r = getHealthReport({
    version: "v1.2.05",
...
test('probeGmailAuth: getProfile resolves → "ok"', async () => {
  const { probeGmailAuth } = await import(
    healthModulePath + "?probe1=" + Date.now()
  );
  const gmail = {
    users: { getProfile: async () => ({ data: { emailAddress: "x@y.com" } }) },
  };
...
test('probeGmailAuth: invalid_grant → "invalid" (the F25 class the file check misses)', async () => {
  const { probeGmailAuth } = await import(
    healthModulePath + "?probe2=" + Date.now()
  );
  const gmail = {
    users: {
      getProfile: async () => {
...
test('probeGmailAuth: 401 → "invalid"', async () => {
  const { probeGmailAuth } = await import(
    healthModulePath + "?probe3=" + Date.now()
  );
  const gmail = {
    users: {
      getProfile: async () => {
...
test('probeGmailAuth: transient/network error → "error" (not "invalid")', async () => {
  const { probeGmailAuth } = await import(
    healthModulePath + "?probe4=" + Date.now()
  );
  const gmail = {
    users: {
      getProfile: async () => {
...
test('getHealthReport: liveAuth "invalid" → degraded (503), checks.tokenLive set', async () => {
  const { getHealthReport } = await import(
    healthModulePath + "?live1=" + Date.now()
  );
  ...
  const r = getHealthReport({
    ...
    settings: {
      schedulerEnabled: true,
      schedulerIntervalHours: 2,
      schedulerLastRunAt: "2026-06-26T11:30:00Z",
    },
    tokenState: "ok",
    configState: "ok",
    liveAuth: "invalid",
  ...
  assert.equal(
    r.ok,
    false,
    "rejected live auth → degraded even though the token file looks ok",
  );
...
test('getHealthReport: liveAuth "error" reported but does NOT fail (no flapping)', async () => {
  const { getHealthReport } = await import(
    healthModulePath + "?live2=" + Date.now()
  );
  const now = Date.parse("2026-06-26T12:00:00Z");
  const r = getHealthReport({
    version: "v1.2.16",
  ...
  assert.equal(
    r.ok,
    true,
    "transient live-probe error must not flap health to degraded",
  );
...
test("getHealthReport: liveAuth absent → no tokenLive check (probe disabled, back-compat)", async () => {
  const { getHealthReport } = await import(
    healthModulePath + "?live3=" + Date.now()
  );
  const now = Date.parse("2026-06-26T12:00:00Z");
  const r = getHealthReport({
    version: "v1.2.16",
  ...
  assert.equal(r.ok, true);
  assert.equal(
    "tokenLive" in r.body.checks,
    false,
    "no tokenLive key when probe disabled",
  );
});
...
test('readWebAsset: returns "ok" when index.html exists in webDist', async () => {
  ...
  try {
    ...
    const { readWebAsset } = await import(
      healthModulePath + "?wa1=" + Date.now()
    );
...
test('readWebAsset: returns "disabled" without touching fs when webEnabled is false', async () => {
  const { readWebAsset } = await import(
    healthModulePath + "?wa3=" + Date.now()
  );
  // Pass a non-existent path — would throw if fs were called
  assert.equal(
    readWebAsset("/nonexistent/path/that/does/not/exist", false),
    "disabled",
  );
});
...
test("resolveWebDist: LOCAL layout — triage.js in app/, bundle at ../web/dist", async () => {
  const { resolveWebDist } = await import(
    healthModulePath + "?rwd1=" + Date.now()
  );
  ...
  try {
    ...
    assert.equal(
      path.resolve(resolveWebDist(moduleDir)),
...
test("resolveWebDist: CONTAINER layout — triage.js at /app, bundle at /app/web/dist", async () => {
  const { resolveWebDist } = await import(
    healthModulePath + "?rwd2=" + Date.now()
  );
...
test("resolveWebDist: neither candidate has index.html → falls back to ../web/dist", async () => {
  const { resolveWebDist } = await import(
    healthModulePath + "?rwd3=" + Date.now()
  );
