...
import { createCalendarEvent, getCalendarClient } from "./lib/calendar.js";
import { analyzeEmail } from "./lib/claude.js";
import {
  sendEventsEmail,
  sortGroupKeysByLocationOrder,
} from "./lib/eventSearch.js";
import {
  ignoreFoundEvent,
  loadFoundEvents,
...
import { addToStats, loadStats, resetStats } from "./lib/stats.js";
import {
  ACTION_DISPATCH,
  filterHidden,
  normalizeGuard,
...
function getListConflicts(viplist, oklist, blocklist) {
  ...
  return Object.values(byEmail)
    .filter((e) => e.lists.size > 1)
    .map((e) => ({ ...e, lists: [...e.lists] }));
}
...
async function renderLegacyHome(req, res) {
  ...
  res.send(
    shell("Gmail Triage", homePage(bl, vip, ok, delPendSummary, listConflicts)),
  );
}
app.get("/legacy", async (req, res) => {
  await renderLegacyHome(req, res);
});
...
app.get("/triage", async (req, res) => {
  try {
    ...
    const scheduledResults = loadScanLog().filter(
      (e) =>
        e.runAt &&
        (!lastTriageAt || new Date(e.runAt) > new Date(lastTriageAt)),
    );
    ...
    snapshotInboxSize(gmail)
      .then((size) => {
        if (size !== null) addToStats({ inboxSize: size });
      })
      .catch(() => {});
    const filtered = emails.filter(
      (e) => !isBlocked(extractEmail(e.from), extractName(e.from)),
    );
    const { body, script } = triagePage(
      filtered,
      blocklist,
      savedStats,
      scanResults,
...
app.post("/blocklist/remove", (req, res) => {
  removeFromBlocklist(req.body.email, req.body.name);
  res.redirect("/blocklist");
});

// ─── VIP List Manager ──────────────────────────────────────────────────────────
app.get("/viplist", (req, res) => {
  res.send(
    shell(
      "VIP List",
      viplistPage(loadViplist().sort((a, b) => a.email.localeCompare(b.email))),
    ),
  );
});
...
app.post("/viplist/remove", (req, res) => {
  removeFromViplist(req.body.email, req.body.name || null);
  res.redirect("/viplist");
});

// ─── OK List Manager ───────────────────────────────────────────────────────────
app.get("/oklist", (req, res) => {
  res.send(
    shell(
      "OK List",
      oklistPage(loadOklist().sort((a, b) => a.email.localeCompare(b.email))),
    ),
  );
});
...
app.post("/oklist/remove", (req, res) => {
  removeFromOklist(req.body.email, req.body.name || null);
  res.redirect("/oklist");
});
...
app.get("/lists", (req, res) => {
  try {
    const { body, script } = listsPage(
      loadBlocklist(),
      loadViplist(),
      loadOklist(),
      loadBlocklistBackup(),
      loadNamedBackups(),
      loadSettings().listsViewMode,
    );
    ...
    const batches = results
      ...
      .map((r) => {
        ...
        if (list === "rules")
          return {
            addLabelIds: [r.label],
            removeLabelIds: r.skipInbox ? ["INBOX", "UNREAD"] : [],
            ids: r.ids,
          };
        return {
          addLabelIds: [list === "vip" ? "..VIP" : "..OK"],
          removeLabelIds: [],
          ids: r.ids,
        }; // tier (vip/ok)
      });
...
async function selectNextTriageMessage(
  gmail,
  { seenSenders, seenIds, hideListed },
) {
    ...
    for (const msg of messages) {
      ...
      if (isBlocked(fromEmail, fromName)) {
        try {
          ...
          autoCleanedEntries.push({
            email: fromEmail,
            reason: "blocklist",
            moved: 1,
            latestEmailDate: new Date(g("Date")).getTime() || Date.now(),
            subjects: [g("Subject") || "(no subject)"],
            ts: Date.now(),
          });
        } catch (e) {
          console.error("auto-clean failed:", e.message);
        }
        continue;
      }
...
app.get("/api/next", async (req, res) => {
  ...
  try {
    const gmail = await getGmailClient();
    const result = await selectNextTriageMessage(gmail, {
      seenSenders,
      seenIds,
      hideListed,
    });
    if (!result)
      return res.json({ html: null, autoCleaned: 0, autoCleanedEntries: [] });
    const {
      id,
      from,
      subject,
    ...
    res.json({ html: null, autoCleaned: 0, autoCleanedEntries: [] });
  }
});
...
app.post("/api/tier", async (req, res) => {
  ...
  try {
    ...
    const alreadyListed = isVip
      ? isViplisted(fromEmail, fromName || null)
...
app.post("/api/ok-clean", async (req, res) => {
  ...
  try {
    ...
    const { cleaned } = await keepAndClean(
      gmail,
      id,
      fromEmail,
      fromName || null,
...
app.post("/api/delete-many", async (req, res) => {
  ...
  if (!Array.isArray(ids) || !ids.length)
    return res.json({ ok: true, trashed: 0 });
  try {
    const gmail = await getGmailClient();
...
async function buildPreviewDocument(gmail, id, { noMeta = false } = {}) {
  ...
  const meta = noMeta
    ? ""
    : "<div class='meta'><div><strong>From:</strong> " +
      g("From").replace(/</g, "&lt;") +
...
app.post("/api/review", async (req, res) => {
  ...
  try {
    ...
    const g = (n) => headers.find((x) => x.name === n)?.value || "";
    const plainRaw =
      findPart(msg.data.payload, "text/plain") ||
      findPart(msg.data.payload, "text/html");
    const body = plainRaw
      ? Buffer.from(plainRaw, "base64url")
          .toString("utf8")
          .replace(/<[^>]+>/g, " ")
      : "";
    const from = g("From");
    ...
    await gmail.users.messages.batchModify({
      userId: "me",
      requestBody: { ids: [id], addLabelIds: [labelId] },
    });
...
app.get("/settings", (req, res) => {
  try {
    const { body, script } = settingsPage(
      loadSettings(),
      loadBlocklistBackup(),
      loadNamedBackups(),
      loadLog(),
...
app.get("/debug", async (req, res) => {
  ...
  try {
    ...
    for (const n of [".DelPend", "..VIP", "..OK"]) {
      const l = labels.find((x) => x.name === n);
      out.push(l ? "✅ " + n + ": id=" + l.id : "⚠️  " + n + " does not exist");
    }
    const bl = loadBlocklist();
    out.push("\n🚫 Blocklist (" + bl.length + "):");
    for (const e of bl)
...
app.post("/events/search", async (req, res) => {
  try {
    ...
    console.error("events search error:", e.message);
  }
...
app.post("/events/send-email", async (req, res) => {
  try {
    ...
    const active = loadFoundEvents().filter(
      (e) => !e.ignored && (!e.date || e.date >= today),
    );
    await sendEventsEmail(gmail, active, loadSettings());
  } catch (e) {
    console.error("events send-email error:", e.message);
  }
  res.redirect("/events");
});
...
app.post("/events/reset-rebuild", async (req, res) => {
  try {
    ...
    console.error("events reset-rebuild error:", e.message);
  }
  res.redirect("/events");
});
...
app.post("/events/calendar", async (req, res) => {
  ...
  try {
    ...
    const link = await createCalendarEvent(calendar, {
      title,
      date,
      time,
      location,
    ...
    console.error("events calendar error:", e.message);
  }
  res.redirect("/events");
});
...
app.post("/settings/event-interests/edit", (req, res) => {
  if (req.body.old && req.body.new)
    updateEventInterest(req.body.old, req.body.new);
  res.redirect("/settings");
});
...
app.get("/health", async (req, res) => {
  ...
  if (process.env.HEALTH_DEEP_PROBE === "1") {
    try {
      liveAuth = await probeGmailAuth(await getGmailClient());
    } catch (e) {
      liveAuth = isAuthError(e) ? "invalid" : "error";
    }
  }
...
function isAuthError(e) {
  return e?.response?.status === 401 || /invalid_grant/i.test(e?.message || "");
}
...
app.get("/api/triage/queue", async (req, res) => {
  ...
  try {
    ...
    const raw = await fetchEmails(gmail, limit, {
      skipSender,
      maxPages: 10,
      excludeListedLabels: hideListed,
    });
...
app.get("/api/triage/next", async (req, res) => {
  ...
  try {
    ...
    res.json({
      email: shapeTriageEmail(result),
      autoCleaned: result.autoCleanedEntries,
    });
  } catch (e) {
    if (isAuthError(e)) return res.status(503).json({ error: "gmail_auth" });
...
app.get("/api/triage/body", async (req, res) => {
  ...
  try {
    ...
    res
      .type("html")
      .send(await buildPreviewDocument(gmail, id, { noMeta: true }));
  } catch (e) {
    if (isAuthError(e)) return res.status(503).json({ error: "gmail_auth" });
  ...
  try {
    ...
    if (action === "ok" || action === "vip") {
      ...
      if (!alreadyListed) {
        ...
        if (isVip) addToViplist(fromEmail, name);
        else addToOklist(fromEmail, name);
        labeled = await labelSender(
          gmail,
          isVip ? "..VIP" : "..OK",
          fromEmail,
          name,
        ...
        const r = await gmail.users.messages.list({
          userId: "me",
          q: `from:${fromEmail} in:inbox -in:sent -in:trash`,
          maxResults: 500,
        });
        labeled = (r.data.messages || []).length;
      }
    ...
    if (action === "archive") {
      if (threadId) await archiveThread(gmail, threadId);
      else await archiveMessage(gmail, id);
...
app.post("/api/triage/undo", async (req, res) => {
  ...
  try {
    ...
    if (spec.undo === "untrash") {
      ...
      await gmail.users.messages.batchModify({
        userId: "me",
        requestBody: { ids: [d.id], addLabelIds: ["INBOX"] },
      });
      ...
      if (d.addedToList) {
        ...
        else if (ln === "blocklist")
          removeFromBlocklist(d.fromEmail, d.fromName || null);
      }
    }
...
app.get("/api/lists", (req, res) => {
  try {
    const vip = loadViplist(),
      ok = loadOklist(),
      blocklist = loadBlocklist(),
      rules = loadRules();
    const bkp = loadBlocklistBackup();
    const single = bkp
      ? { backedUpAt: bkp.backedUpAt, count: (bkp.list || []).length }
      : null;
    const named = loadNamedBackups().map((b) => ({
      n: b.n,
...
app.post("/api/lists/remove", (req, res) => {
  ...
  try {
    ...
    triageServerError(res, e, "/api/lists/remove");
  }
});
app.post("/api/lists/reset-blocklist", (req, res) => {
  try {
    const backedUp = backupBlocklist();
    resetBlocklist();
    res.json({ ok: true, backedUp });
  } catch (e) {
    triageServerError(res, e, "/api/lists/reset-blocklist");
  }
});
app.post("/api/lists/backup", (req, res) => {
  try {
    res.json({ ok: true, n: createNamedBackup() });
  } catch (e) {
    triageServerError(res, e, "/api/lists/backup");
  }
});
...
app.post("/api/rules/add", (req, res) => {
  ...
  try {
    ...
    triageServerError(res, e, "/api/rules/add");
  }
});
app.post("/api/rules/update", (req, res) => {
  ...
  try {
    ...
    triageServerError(res, e, "/api/rules/update");
  }
});
app.post("/api/rules/toggle", (req, res) => {
  if (!req.body?.id) return res.status(400).json({ error: "Missing id" });
  try {
    res.json({ ok: true, enabled: toggleRule(req.body.id) });
  } catch (e) {
    triageServerError(res, e, "/api/rules/toggle");
  }
});
app.post("/api/rules/delete", (req, res) => {
  ...
  try {
    ...
    triageServerError(res, e, "/api/rules/delete");
  }
});
...
app.get("/api/events", (req, res) => {
  try {
    ...
    const groups = sortGroupKeysByLocationOrder(
      Object.keys(grouped),
      settings.locations || [],
    ...
    res.json({
      ok: true,
      groups,
      lastRunAt: settings.eventsSearchLastRunAt,
      interests,
      hasInterests: interests.length > 0,
    });
  } catch (e) {
    triageServerError(res, e, "/api/events");
  }
});
app.post("/api/events/ignore", (req, res) => {
  try {
    ...
    triageServerError(res, e, "/api/events/ignore");
  }
});
...
app.get("/api/settings", (req, res) => {
  try {
    ...
    triageServerError(res, e, "/api/settings");
  }
});
app.post("/api/settings/scheduler", (req, res) => {
  const { enabled, startHour, startMinute, intervalHours } = req.body || {};
  try {
    setScheduler(enabled, startHour, startMinute, intervalHours);
    res.json({ ok: true });
  } catch (e) {
    triageServerError(res, e, "/api/settings/scheduler");
  }
});
app.post("/api/settings/daily-summary", (req, res) => {
  const { enabled, email } = req.body || {};
  try {
    setDailySummary(enabled, email);
    res.json({ ok: true });
  } catch (e) {
    triageServerError(res, e, "/api/settings/daily-summary");
  }
});
app.post("/api/settings/daily-summary-schedule", (req, res) => {
  ...
  try {
    ...
    triageServerError(res, e, "/api/settings/daily-summary-schedule");
  }
});
app.post("/api/settings/events-search", (req, res) => {
  const { enabled, intervalDays, email } = req.body || {};
  try {
    setEventsSearchSettings(enabled, intervalDays, email);
    res.json({ ok: true });
  } catch (e) {
    triageServerError(res, e, "/api/settings/events-search");
  }
});
app.post("/api/settings/timezone", (req, res) => {
  ...
  try {
    ...
    triageServerError(res, e, "/api/settings/timezone");
  }
});
app.post("/api/settings/lists-view-mode", (req, res) => {
  try {
    ...
    triageServerError(res, e, "/api/settings/lists-view-mode");
  }
});
app.post("/api/settings/bulk-guard-threshold", (req, res) => {
  try {
    setBulkGuardThreshold(req.body?.threshold);
    res.json({
      ok: true,
      threshold: getBulkGuardThreshold(BULK_GUARD_THRESHOLD),
    });
  } catch (e) {
    triageServerError(res, e, "/api/settings/bulk-guard-threshold");
  }
});
app.post("/api/settings/locations/add", (req, res) => {
  ...
  try {
    ...
    triageServerError(res, e, "/api/settings/locations/add");
  }
});
app.post("/api/settings/locations/remove", (req, res) => {
  try {
    removeLocation(req.body?.location);
    res.json({ ok: true });
  } catch (e) {
    triageServerError(res, e, "/api/settings/locations/remove");
  }
});
app.post("/api/settings/event-interests/add", (req, res) => {
  try {
    ...
    triageServerError(res, e, "/api/settings/event-interests/add");
  }
});
app.post("/api/settings/event-interests/remove", (req, res) => {
  try {
    ...
    triageServerError(res, e, "/api/settings/event-interests/remove");
  }
});
app.post("/api/settings/event-interests/edit", (req, res) => {
  const { old: oldTopic, new: newTopic } = req.body || {};
  try {
    if (oldTopic && newTopic) updateEventInterest(oldTopic, newTopic);
    res.json({ ok: true });
  } catch (e) {
    triageServerError(res, e, "/api/settings/event-interests/edit");
  }
});
...
app.post("/api/settings/restore-blocklist-backup", (req, res) => {
  ...
  try {
    const restored = restoreBlocklistBackup(merge === "true" || merge === true);
    res.json({ ok: true, restored });
  } catch (e) {
    triageServerError(res, e, "/api/settings/restore-blocklist-backup");
  }
});
app.post("/api/settings/restore-named-backup", (req, res) => {
  const { n, merge } = req.body || {};
  try {
    const restored = restoreNamedBackup(
      parseInt(n),
      merge === "true" || merge === true,
    );
    res.json({ ok: true, restored });
  } catch (e) {
    triageServerError(res, e, "/api/settings/restore-named-backup");
  }
});
app.post("/api/settings/delete-named-backup", (req, res) => {
  const { n } = req.body || {};
  if (n == null || Number.isNaN(parseInt(n)))
    return res.status(400).json({ error: "Missing or invalid n" });
  try {
    deleteNamedBackup(parseInt(n));
    res.json({ ok: true });
  } catch (e) {
    triageServerError(res, e, "/api/settings/delete-named-backup");
  }
});
...
if (process.env.WEB_APP_ENABLED !== "0") {
  ...
  app.get(/^\/app(\/.*)?$/, (req, res) =>
    res.sendFile(pathmod.join(WEB_DIST, "index.html")),
  );
}
