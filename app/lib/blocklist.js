...
export const isBlocked = (fromEmail, fromName = null) =>
  store.match(fromEmail, fromName);
...
export function backupBlocklist() {
  ...
  atomicWriteFileSync(
    BACKUP_PATH,
    JSON.stringify({ list, backedUpAt: new Date().toISOString() }, null, 2),
  );
  return list.length;
}
export function loadBlocklistBackup() {
  try {
    const raw = JSON.parse(fs.readFileSync(BACKUP_PATH));
    return { list: raw.list || [], backedUpAt: raw.backedUpAt };
  } catch {
    return null;
  }
}
...
export function loadNamedBackups() {
  try {
    return JSON.parse(fs.readFileSync(NAMED_BACKUPS_PATH));
  } catch {
    return [];
  }
}
...
export function deleteNamedBackup(n) {
  atomicWriteFileSync(
    NAMED_BACKUPS_PATH,
    JSON.stringify(
      loadNamedBackups().filter((b) => b.n !== n),
      null,
      2,
    ),
  );
