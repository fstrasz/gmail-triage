...
import { addScannedEmailIds, loadSettings } from "./settings.js";
...
Return [] if nothing found.`;
  ...
  console.log(
    `[eventSearch] web search: response ${text.length} chars, tool_use blocks: ${toolUses}, json match: ${arr ? "yes" : "no"}, stop_reason: ${response.stop_reason}`,
  );
  if (!arr) {
    console.log(
      `[eventSearch] web search response preview (first 400 chars): ${text.slice(0, 400)}`,
    );
    return [];
  }
  try {
    ...
    console.error(
      `[eventSearch] web search JSON head (first 300 chars): ${arr.slice(0, 300)}`,
    );
    console.error(
      `[eventSearch] web search JSON tail (last 300 chars): ${arr.slice(-300)}`,
    );
    return [];
  }
}
...
const BODY_RECORD_EVENTS_TOOL = {
  name: "record_events",
  description:
    "Record every real-world event extracted from the inbox emails in this batch. Call exactly once with all events found. Pass an empty array if no relevant events are found.",
  input_schema: {
    type: "object",
    properties: {
      events: {
        type: "array",
        description:
          "One entry per distinct event. A multi-event newsletter must produce one entry per event, never one entry for the whole newsletter.",
        items: {
          type: "object",
          properties: {
            title: { type: "string" },
            ...
            time: {
              type: ["string", "null"],
              description: "HH:MM 24h, or null if unknown",
            },
            location: { type: "string", description: "venue + city" },
            url: {
              type: "string",
              description:
                "https://mail.google.com/mail/u/0/#all/MESSAGE_ID (the source email id)",
            },
            description: {
              type: "string",
              description:
                "1-2 sentences describing the EVENT itself, no match-explanation phrases",
            },
            interest: {
              type: "string",
              description: "which configured interest matched",
            },
            configuredLocation: {
              type: "string",
              description:
                "which configured Location matched, exactly as written",
            },
...
export function extractBodyScanEvents(response) {
  ...
  try {
    ...
    return {
      events: Array.isArray(parsed) ? parsed : null,
      source: "text",
      text,
    };
  } catch (e) {
...
export function getEmailBodyText(payload) {
  ...
  if (plain && stripped)
    return plain.length >= stripped.length ? plain : stripped;
      ...
      if (size > 0 && size < VISION_MAX_IMAGE_BYTES) {
        ...
        if (!data && part.body.attachmentId) {
          try {
            ...
            console.error(
              "fetchEmailImages: attachment fetch failed:",
              e.message,
            );
          }
        }
...
export function scoreCandidateByTitle(candidate, eventTitle) {
  ...
  const meaningful = titleWords.filter(
...
async function extractCanonicalLink(
  gmail,
  messageId,
  eventTitle,
  client,
  ...
  try {
    ...
    const viewInBrowser = anchors.find(
      (a) => a.text && VIEW_IN_BROWSER_RE.test(a.text),
    );
    ...
    for (const a of filtered)
      if (!seen.has(a.href)) seen.set(a.href, a.text || "");
    ...
    if (decision.kind === "direct") {
      console.log(
        `[eventSearch] canonical (rank-only) for "${eventTitle}": ${decision.href}${decision.score != null ? " score=" + decision.score : ""}`,
      );
    ...
    console.error(
      `extractCanonicalLink failed for "${eventTitle}":`,
      e.message,
    );
    return null;
  }
...
No other text, no explanation.`,
      ...
      if (dateMatch) {
        ...
        if (candidate >= today && candidate <= horizon) {
          ...
          console.log(
            `[eventSearch] image-derived date for "${event.title}": ${candidate}`,
          );
        }
      }
    } catch (e) {
      console.error(
        `enrichDatesFromImages failed for "${event.title}":`,
        e.message,
      );
    }
  });
...
export async function scanEmailsForEvents(gmail, interests, locations) {
  ...
  const listRes = await gmail.users.messages.list({
    userId: "me",
    q: 'in:inbox newer_than:3d -subject:"Gmail Triage" -label:.DelPend',
    maxResults: 100,
  });
  ...
  console.log(
    `[eventSearch] email scan: ${allMessages.length} recent (excluding .DelPend), ${messages.length} new to scan`,
  );
  ...
  const bodies = await Promise.all(
    messages.map(async (m) => {
      ...
      if (body.length > MAX_BODY_CHARS_PER_EMAIL) {
        body =
          body.slice(0, MAX_BODY_CHARS_PER_EMAIL) +
          "\n[…body truncated at " +
  ...
  console.log(
    `[eventSearch] fetched ${bodies.length} bodies, total ~${bodies.reduce((s, b) => s + b.tokens, 0)} tokens`,
  );
  ...
  console.log(
    `[eventSearch] body scan: ${bodies.length} emails packed into ${batches.length} batch(es) (cap ${BODY_MAX_EMAILS_PER_BATCH}/batch)`,
  );
...
Return the JSON array per the schema above. Empty array if no events.`;
    try {
      ...
      const {
        ...
        text: t = "",
        parseError,
      } = extractBodyScanEvents(r);
      console.log(
        `[eventSearch] body batch ${bi + 1}/${batches.length}: ${batch.length} emails, source: ${source}, stop_reason: ${r.stop_reason}, events: ${parsed ? parsed.length : "parse-failed"}`,
      );
      if (!parsed) {
        if (parseError)
          console.error(
            `[eventSearch] body batch ${bi + 1}: JSON parse failed: ${parseError}`,
          );
        console.warn(
          `[eventSearch] body batch ${bi + 1}: response preview: ${t.slice(0, 400)}`,
        );
        continue;
      }
      console.log(
        `[eventSearch] body batch ${bi + 1}: extracted ${parsed.length} event(s)`,
      );
      ...
      if (parsed.length === 0) {
        if (t)
          console.warn(
            `[eventSearch] body batch ${bi + 1}: ZERO events — preamble (first 500 chars): ${t.slice(0, 500).replace(/\s+/g, " ")}`,
          );
        for (const [ei, b] of batch.entries()) {
          const excerpt = (b.body || "").slice(0, 250).replace(/\s+/g, " ");
          console.warn(
            `[eventSearch]   email ${ei + 1}/${batch.length} id=${b.id} subj="${b.subject}" body[${b.body.length}ch]: ${excerpt}`,
          );
        }
      }
    } catch (e) {
  ...
  console.log(
    `[eventSearch] body scan: extracted ${allEvents.length} event(s) total, ${successfulIds.size}/${messages.length} emails marked scanned`,
  );
  // Cost summary — Sonnet 4.6 rates: $3/MTok input, $3.75/MTok cache write, $0.30/MTok cache read, $15/MTok output
  const cost =
    (usage.input * 3 +
      usage.cache_creation * 3.75 +
      usage.cache_read * 0.3 +
      usage.output * 15) /
    1_000_000;
  console.log(
    `[eventSearch] body scan tokens: ${usage.input} input, ${usage.cache_creation} cache_creation, ${usage.cache_read} cache_read, ${usage.output} output — est $${cost.toFixed(4)}`,
  );
...
function renderEventItem(e) {
  const priceRating = [
    e.pricePerPerson
      ? `<strong style="color:#16a34a">${e.pricePerPerson} / person</strong>`
      : "",
    e.rating ? `&#11088; ${e.rating}` : "",
  ]
    .filter(Boolean)
...
export function shouldSkipWebSearch(
  now,
  lastRunIso,
  minIntervalMs = WEB_SEARCH_MIN_INTERVAL_MS,
) {
...
export async function sendEventsEmail(gmail, events, settings) {
  ...
  if (!rawTo) {
    console.warn(
      "[eventSearch] no recipient configured — set Events Email in Settings",
    );
    return;
  }
  const to = rawTo
