...
beforeEach(() => {
  ...
  api.getQueue.mockResolvedValue({
    emails: [makeEmail("e1"), makeEmail("e2")],
    counts: { left: 2 },
  });
});
...
describe("TriagePage / real React-Query interleave", () => {
  test("guard: card is restored to the top of the deck and GuardDialog opens after an ASYNC guard resolve", async () => {
    ...
    api.getQueue.mockResolvedValueOnce({
      emails: [makeEmail("e2")],
      counts: { left: 1 },
    });
    api.postAction.mockResolvedValueOnce({
      ok: true,
      undo: stubUndo("archive"),
    });
    fireEvent.click(screen.getByRole("button", { name: /^confirm$/i }));

    await waitFor(() => {
      const last = api.postAction.mock.calls.at(-1)![0] as {
        confirmed?: boolean;
        action: string;
        id: string;
      };
