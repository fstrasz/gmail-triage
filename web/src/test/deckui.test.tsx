...
describe("TriagePage / Deck UI", () => {
  ...
  test("3. action surface (DECK-3, hidden): OK/Archive/Review are buttons, VIP&Clean/Unsub in More, the 4 swipe actions on the card legend", () => {
    ...
    for (const a of ["ok", "archive", "review"]) {
      expect(
        screen.queryByRole("button", { name: ACTION_LABELS[a] }),
      ).not.toBeNull();
    }
    ...
    expect(
      within(dialog).queryByRole("menuitem", {
        name: ACTION_LABELS["vip-clean"],
      }),
    ).not.toBeNull();
    expect(
      within(dialog).queryByRole("menuitem", { name: ACTION_LABELS.unsub }),
    ).not.toBeNull();
  });
  ...
  test("4. a guard result opens GuardDialog; confirming re-calls the action with confirmed:true", () => {
    hookState.actionResult = {
      ok: false,
      guard: { count: 250, message: "250 emails will be archived" },
    };
    render(<TriagePage />);
    ...
    expect(
      screen.getByText(/250 emails will be archived/i),
    ).toBeInTheDocument();
    ...
    const last = actionMutate.mock.calls.at(-1)![0] as {
      confirmed?: boolean;
      action: string;
    };
  ...
  test('6b. genuinely-empty (shown mode) state shows "Inbox triaged"', () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /hide.*(vip|ok|listed)/i }),
    );
  ...
  test("7b. honest bulk copy: vip-clean toast says N archived + undo removes from list", () => {
    ...
    const dialog = screen.getByRole("dialog");
    fireEvent.click(
      within(dialog).getByRole("menuitem", {
        name: ACTION_LABELS["vip-clean"],
      }),
    );
    const toast = screen.getByRole("status");
    ...
    expect(
      within(toast).getByText(/undo removes from list/i),
    ).toBeInTheDocument();
  });
  ...
  test('7c. honest unsub copy (FIX H3): toast does NOT claim "undo available" and renders NO Undo button', () => {
    ...
    const dialog = screen.getByRole("dialog");
    fireEvent.click(
      within(dialog).getByRole("menuitem", { name: ACTION_LABELS.unsub }),
    );
    const toast = screen.getByRole("status");
    expect(
      within(toast).queryByText(/undo available/i),
    ).not.toBeInTheDocument();
    expect(
      within(toast).queryByRole("button", { name: /undo/i }),
    ).not.toBeInTheDocument();
  ...
  test("9. body renders ONLY as a sandboxed iframe (no dangerouslySetInnerHTML), with View all from sender link", () => {
    ...
    // Expand the body.
    fireEvent.click(
      screen.getByRole("button", { name: /show (email )?body|expand/i }),
    );
  ...
  test("9b. peek (background) cards are inert (FIX C, WCAG 4.1.2) while the top card is not", () => {
    ...
    const topLink = screen
      .getAllByRole("link", { name: /view all from .*sender/i })
      .find((a) => a.closest("[inert]") === null);
  ...
  test("11. desktop (mouse, no swipe): every action is a visible button, no ⋯ menu", () => {
    ...
    try {
      ...
      for (const a of Object.keys(ACTION_LABELS)) {
        expect(
          screen.getByRole("button", { name: ACTION_LABELS[a] }),
        ).toBeInTheDocument();
      }
