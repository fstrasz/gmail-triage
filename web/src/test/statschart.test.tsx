...
function day(date: string, over: Partial<StatsDay> = {}): StatsDay {
  return { date, kept: 0, cleaned: 0, junked: 0, unsubbed: 0, vip: 0, ok: 0, inboxSize: null, ...over }
}
...
describe('StatsChart', () => {
  test('renders 30 bars, carries in-window fixture values, and drops the out-of-window day', () => {
    const daily: StatsDay[] = [
      ...
      day(daysAgo(29), { kept: 1, cleaned: 2, junked: 3, unsubbed: 4, vip: 5, ok: 6 }), // total 21
      day(daysAgo(10), { kept: 2, cleaned: 0, junked: 1, unsubbed: 0, vip: 0, ok: 0 }), // total 3
      day(daysAgo(0), { kept: 0, cleaned: 0, junked: 0, unsubbed: 0, vip: 7, ok: 0 }), // total 7
      // 30 days ago is one day OUTSIDE the window (loop only covers i=29..0).
      day(daysAgo(30), { kept: 9, cleaned: 9, junked: 9, unsubbed: 9, vip: 9, ok: 9 }),
    ]
    ...
    expect(bars.map((b) => b.getAttribute('data-date'))).not.toContain(daysAgo(30))

    const byDate = (date: string) => bars.find((b) => b.getAttribute('data-date') === date)!
    ...
    const zeroFilled = bars.filter(
      (b) => ![daysAgo(29), daysAgo(10), daysAgo(0)].includes(b.getAttribute('data-date')!),
    )
