...
import { useReview, useExecute, useCalendar, useDismiss } from './reviewQueries.ts'
...
export function ReviewPage() {
  ...
  const active = ordered.find((i) => i.id === selectedId) ?? (desktop ? ordered[0] ?? null : null)
  ...
  async function createAll(id: string, entries: { event: ReviewEvent; idx: number }[]) {
  ...
  const detail = active ? (
    <ReviewDetail
      ...
      onCalendar={(id, eventIndex, event) => calendar.mutate({ id, eventIndex, event })}
      onCreateAll={(entries) => { void createAll(active.id, entries) }}
    ...
    return (
      <div className="flex h-full flex-col p-4">
        ...
        <div className="flex flex-1 overflow-hidden rounded-2xl border border-hairline">
          <aside className="w-72 flex-shrink-0 overflow-y-auto border-r border-hairline">{list}</aside>
          <div className="min-w-0 flex-1 overflow-hidden bg-white">
            {detail ?? <p className="p-4 text-sm text-muted">Select a review item.</p>}
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="flex h-full flex-col p-4">
      ...
      {active ? (
        ...
        <div className="flex-1 overflow-y-auto rounded-2xl border border-hairline">{list}</div>
      )}
    </div>
  )
}
...
function ReviewSkeleton() {
  return (
    <div className="flex h-full flex-col p-4">
      <Header count={0} />
      <div data-testid="review-skeleton" className="flex-1 animate-pulse rounded-2xl border border-hairline bg-hairline/40" />
    </div>
  )
}
...
function EmptyReview() {
  return (
    <div className="flex h-full flex-col items-center justify-center text-center text-muted">
      ...
      <p className="mt-1 text-sm">Queue an email for Claude review from the triage screen.</p>
    </div>
  )
}
...
function ReconnectGmail() {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-3 p-4 text-center">
      ...
      <p className="text-sm text-muted">The review queue could not be loaded. Re-authorize to continue.</p>
