...
import { getReview, execute, addCalendar, dismiss } from './reviewApi.ts'
...
export function useExecute() {
  ...
  return useMutation({
    mutationFn: ({ id, action }: { id: string; action: ReviewAction }) => execute(id, action),
    onSuccess: () => {
...
export function useCalendar() {
  const queryClient = useQueryClient()
  return useMutation({
    mutationFn: ({ id, eventIndex, event }: { id: string; eventIndex: number; event: ReviewEvent }) =>
