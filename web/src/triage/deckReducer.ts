...
function activeId(state: DeckState, explicit?: string): string | null {
  ...
  if (state.selectedId && state.cards.some((c) => c.id === state.selectedId))
...
export function deckReducer(state: DeckState, event: DeckEvent): DeckState {
  switch (event.type) {
    ...
    case "select":
      return state.cards.some((c) => c.id === event.id)
    ...
    case "advance": {
      ...
      const nextSelected = cards.length
        ? cards[Math.min(index, cards.length - 1)].id
        : null;
      return {
        ...state,
        cards,
        removed: [{ card, index }, ...state.removed],
