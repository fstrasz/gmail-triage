...
describe("list mutations", () => {
  ...
  test("resetBlocklist POSTs to /api/lists/reset-blocklist", async () => {
    globalThis.fetch = vi
      .fn()
      .mockResolvedValueOnce(jsonRes(200, { ok: true, backedUp: 4 }));
  ...
  test("toggleRule POSTs the rule id", async () => {
    globalThis.fetch = vi
      .fn()
      .mockResolvedValueOnce(jsonRes(200, { ok: true, enabled: false }));
  ...
  test("maps 503 gmail_auth to the auth-error branch (does not throw)", async () => {
    globalThis.fetch = vi
      .fn()
      .mockResolvedValueOnce(jsonRes(503, { error: "gmail_auth" }));
  ...
  test("throws on an unexpected non-2xx (500)", async () => {
    ...
    await expect(
      addSender({ list: "vip", email: "a@b.com" }),
    ).rejects.toThrow();
  });
});
...
describe("runReapply", () => {
  ...
  test("resolves an under-threshold done JSON", async () => {
    globalThis.fetch = vi
      ...
      .mockResolvedValueOnce(
        jsonRes(200, { ok: true, list: "vip", totalLabeled: 0, results: [] }),
      );
  ...
  test("reapplyUndo returns the parsed result", async () => {
    globalThis.fetch = vi
      .fn()
      .mockResolvedValueOnce(
        jsonRes(200, { ok: true, list: "vip", reversed: 3, caveat: null }),
      );
...
describe("mergeLists / filterRows / chipCounts", () => {
  test("merges by lowercased email with per-list memberships, sorted", () => {
    const rows = mergeLists({
      ...
      blocklist: [
        { email: "a@x.com", name: "Alan", reason: "spam", date: "2026-01-03" },
      ],
    });
    expect(rows.map((r) => r.email)).toEqual(["a@x.com", "b@x.com"]);
    expect(rows[0].memberships.map((m) => m.list).sort()).toEqual([
      "blocklist",
      "vip",
    ]);
    expect(
      rows[0].memberships.find((m) => m.list === "blocklist")?.reason,
    ).toBe("spam");
  });
  ...
  test("chipCounts + filterRows", () => {
    ...
    expect(filterRows(rows, "vip", "").map((r) => r.email)).toEqual([
      "a@x.com",
    ]);
    expect(filterRows(rows, "all", "b@").map((r) => r.email)).toEqual([
      "b@x.com",
    ]);
  });
});
