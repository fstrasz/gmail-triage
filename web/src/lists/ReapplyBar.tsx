...
import { runReapply, reapplyUndo } from './listsApi.ts'
...
export function ReapplyBar() {
  ...
  const [guard, setGuard] = useState<(GuardInfo & { list: ReapplyList }) | null>(null)
  const [undoInfo, setUndoInfo] = useState<{ list: ReapplyList; count: number } | null>(null)
  ...
  async function start(list: ReapplyList, confirmed = false) {
    ...
    const result = await runReapply(list, {
      confirmed,
      onProgress: (p) => setProgress(`${p.current}/${p.total} ${p.email ?? ''}`.trim()),
    })
    setRunning(null)
    setProgress('')
  ...
  async function undo() {
    ...
    setMessage(r.ok ? `Undone: ${r.reversed} reversed${r.caveat ? ` — ${r.caveat}` : ''}` : `Undo failed: ${r.error}`)
  }
  ...
  return (
    <section className="flex flex-col gap-2 rounded-xl border border-hairline p-3">
      ...
      <GuardDialog guard={guard} onConfirm={confirmGuard} onCancel={() => setGuard(null)} />
    </section>
  )
}
