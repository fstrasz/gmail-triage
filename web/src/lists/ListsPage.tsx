...
import { mergeLists, filterRows, chipCounts } from './listsApi.ts'
...
const FILTER_LABEL: Record<Filter, string> = { all: 'All', blocklist: 'Blocked', vip: 'VIP', ok: 'OK' }
  ...
  return (
    <div className="flex h-full flex-col gap-6 overflow-y-auto p-4">
      ...
      <section className="flex flex-col gap-3">
          ...
          {FILTERS.map((f) => (
            <button
              ...
              className={`rounded-full border px-3 py-1 text-sm font-medium ${
                filter === f ? 'border-ink bg-ink text-white' : 'border-hairline text-muted'
              }`}
            >
        ...
        {visible.length === 0 ? (
          <p data-testid="lists-empty" className="rounded-xl border border-hairline p-6 text-center text-sm text-muted">
            {rows.length === 0 ? 'No senders on any list yet.' : 'No senders match this filter.'}
          </p>
        ) : (
          <ul className="divide-y divide-hairline rounded-xl border border-hairline">
            {visible.map((row) => (
              <ListRow key={row.email} row={row} onRemove={onRemove} removing={removeSender.isPending} />
            ))}
          </ul>
        )}
      </section>
...
function ReconnectGmail() {
  return (
    <div className="flex h-full flex-col items-center justify-center gap-3 p-4 text-center">
      ...
      <p className="text-sm text-muted">The Gmail connection expired. Re-authorize to load your lists.</p>
