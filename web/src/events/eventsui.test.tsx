...
const addCalMutateAsync = vi
  .fn()
  .mockResolvedValue({ ok: true, url: "https://cal/x" });
...
describe("EventsPage", () => {
  ...
  test("8. canonicalUrl wins over url for the title link", () => {
    loaded([
      {
        location: "Reno, NV",
        events: [makeEvent("e2", { canonicalUrl: "https://canon/e2" })],
      },
    ]);
    renderPage();
