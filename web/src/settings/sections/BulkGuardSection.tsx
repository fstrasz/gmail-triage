  ...
  return (
    <Card title="Bulk-Guard Threshold">
      <p className="mb-2 text-sm text-muted">
        Confirmation is required for a bulk action affecting more than this many emails. Current effective value:{' '}
        <span className="font-semibold text-ink">{threshold}</span>. Leave blank or 0 to use the default (
        {DEFAULT_THRESHOLD}).
      </p>
      <form className="flex items-center gap-2" onSubmit={onSave}>
        ...
        />
        <button type="submit" className={`${saveBtnClass} mt-0`} disabled={save.isPending}>
          Save
        </button>
      </form>
    </Card>
