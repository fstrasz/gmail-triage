...
export function BackupsSection({ backups }: { backups: Backups }) {
  ...
  function confirm() {
    ...
    else if (pending.kind === 'restore-named') restoreNamed.mutate({ n: pending.n, merge })
  ...
  const dialogCopy = (() => {
    ...
    if (pending.kind === 'delete-named') {
      return { title: 'Delete backup', message: `Delete named backup #${pending.n}? This cannot be undone.`, confirmLabel: 'Delete' }
    }
    const target = pending.kind === 'restore-single' ? 'the auto-backup' : `named backup #${pending.n}`
    return {
  ...
  return (
    <Card title="Blocklist Backups">
      ...
      <div className="mb-3">
        <h3 className="text-xs font-semibold uppercase tracking-wide text-muted">Auto-backup</h3>
        {backups.single ? (
          <div className="mt-1 flex items-center justify-between gap-2 text-sm text-ink">
            <span>
              {backups.single.count} entries — {new Date(backups.single.backedUpAt).toLocaleString()}
            </span>
            <button type="button" className="font-semibold text-ink underline" onClick={() => setPending({ kind: 'restore-single' })}>
              Restore
            </button>
          </div>
        ) : (
      ...
      <div>
        <h3 className="text-xs font-semibold uppercase tracking-wide text-muted">Named backups</h3>
        {backups.named.length === 0 ? (
          <p className="mt-1 text-sm text-muted">No named backups.</p>
        ) : (
          <ul className="mt-1 flex flex-col gap-1">
            {backups.named.map((b) => (
              <li key={b.n} className="flex items-center justify-between gap-2 text-sm text-ink">
                <span>
                  #{b.n} — {b.count} entries — {new Date(b.backedUpAt).toLocaleString()}
                </span>
                <span className="flex gap-3">
                  <button type="button" className="font-semibold text-ink underline" onClick={() => setPending({ kind: 'restore-named', n: b.n })}>
                    Restore
                  </button>
                  <button type="button" className="text-junk underline" onClick={() => setPending({ kind: 'delete-named', n: b.n })}>
                    Delete
                  </button>
                </span>
              </li>
