  ...
  return (
    <Card title="Locations">
      <ul className="mb-3 flex flex-wrap gap-2">
        {locations.length === 0 && <li className="text-sm text-muted">No locations yet.</li>}
        {locations.map((loc) => (
          <li key={loc} className="flex items-center gap-1 rounded-full border border-hairline px-3 py-1 text-sm text-ink">
            <span>{loc}</span>
            <button
              type="button"
