...
import { useAddInterest, useRemoveInterest, useEditInterest } from '../settingsQueries.ts'
...
export function InterestsSection({ interests }: { interests: string[] }) {
  ...
  function saveEdit(e: FormEvent) {
    ...
    if (editing && next && next !== editing) edit.mutate({ old: editing, new: next })
    setEditing(null)
  }
  ...
  return (
    <Card title="Event Interests">
      <ul className="mb-3 flex flex-col gap-2">
        {interests.length === 0 && <li className="text-sm text-muted">No interests yet.</li>}
        {interests.map((topic) => (
            ...
            {editing === topic ? (
              <form className="flex flex-1 gap-2" onSubmit={saveEdit}>
                <input
                  ...
                  autoFocus
                />
                <button type="submit" className="text-sm font-semibold text-ink">Save</button>
                <button type="button" className="text-sm text-muted" onClick={() => setEditing(null)}>Cancel</button>
              </form>
            ) : (
              <>
                <span className="flex-1">{topic}</span>
                <button type="button" className="text-muted hover:text-ink" onClick={() => startEdit(topic)}>Edit</button>
                <button
                  type="button"
