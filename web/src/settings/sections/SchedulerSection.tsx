...
import { useSetScheduler, useRunScan } from '../settingsQueries.ts'
...
export function SchedulerSection({ settings }: { settings: Settings }) {
  ...
  const [intervalHours, setIntervalHours] = useState(settings.schedulerIntervalHours)
    ...
    <Card title="Auto-Clean Scheduler">
      <form onSubmit={onSave}>
        ...
        <Field label="Start hour (0-23)">
          <input type="number" min={0} max={23} className={numberInputClass} value={startHour}
            onChange={(e) => setStartHour(Number(e.target.value))} />
        </Field>
        <Field label="Start minute (0-59)">
          <input type="number" min={0} max={59} className={numberInputClass} value={startMinute}
            onChange={(e) => setStartMinute(Number(e.target.value))} />
        </Field>
        <Field label="Interval (hours)">
          <input type="number" min={0.5} step={0.5} className={numberInputClass} value={intervalHours}
      ...
      <div className="mt-4 border-t border-hairline pt-3">
        ...
        {scan && scan.ok && (
          <p className="mt-2 text-sm text-muted" role="status">
            Moved {scan.totalMoved} ({scan.blocklistMoved} block / {scan.vipMoved} VIP / {scan.okMoved} OK /{' '}
            {scan.rulesMoved} rules) at {scan.timeLabel}.
          </p>
        )}
        {scan && !scan.ok && (
          <p className="mt-2 text-sm text-junk" role="status">
            Gmail connection expired. <a href="/auth" className="font-semibold underline">Reconnect</a>.
          </p>
        )}
      </div>
    </Card>
