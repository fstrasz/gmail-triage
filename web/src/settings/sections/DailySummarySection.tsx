...
import { useSetDailySummary, useSetDailySummarySchedule } from '../settingsQueries.ts'
import type { Settings } from '../settingsApi.ts'

const UNITS: Settings['dailySummaryIntervalUnit'][] = ['hours', 'days', 'weeks']
...
export function DailySummarySection({ settings }: { settings: Settings }) {
  ...
  const [intervalValue, setIntervalValue] = useState(settings.dailySummaryIntervalValue)
  const [intervalUnit, setIntervalUnit] = useState(settings.dailySummaryIntervalUnit)
  ...
  return (
    <Card title="Daily Summary">
      <form onSubmit={onSaveSummary}>
        ...
        <Field label="Email">
          <input type="email" className={`${inputClass} w-56`} value={email}
            onChange={(e) => setEmail(e.target.value)} placeholder="recipient@example.com" />
        </Field>
        <button type="submit" className={saveBtnClass} disabled={saveSummary.isPending}>
          Save summary
        </button>
      </form>

      <form className="mt-4 border-t border-hairline pt-3" onSubmit={onSaveSchedule}>
        <Field label="Hour (0-23)">
          <input type="number" min={0} max={23} className={numberInputClass} value={hour}
            onChange={(e) => setHour(Number(e.target.value))} />
        </Field>
        <Field label="Minute (0-59)">
          <input type="number" min={0} max={59} className={numberInputClass} value={minute}
            onChange={(e) => setMinute(Number(e.target.value))} />
        </Field>
        <Field label="Every">
          <span className="flex items-center gap-2">
            <input type="number" min={1} className={numberInputClass} value={intervalValue}
              onChange={(e) => setIntervalValue(Number(e.target.value))} />
            <select
              className={inputClass}
              value={intervalUnit}
              onChange={(e) => setIntervalUnit(e.target.value as Settings['dailySummaryIntervalUnit'])}
        ...
        <button type="submit" className={saveBtnClass} disabled={saveSchedule.isPending}>
          Save schedule
        </button>
      </form>
    </Card>
