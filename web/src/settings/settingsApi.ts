...
export function setListsViewMode(
  mode: "table" | "compact",
): Promise<{ ok: true }> {
  return postJson("/api/settings/lists-view-mode", { mode });
}

export function setBulkGuardThreshold(
  threshold: number,
): Promise<{ ok: true; threshold: number }> {
  return postJson<{ ok: true; threshold: number }>(
    "/api/settings/bulk-guard-threshold",
    { threshold },
  );
}
...
export function editEventInterest(v: {
  old: string;
  new: string;
}): Promise<{ ok: true }> {
...
export function restoreNamedBackup(v: {
  n: number;
  merge: boolean;
}): Promise<{ ok: true; restored?: number }> {
